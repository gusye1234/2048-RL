...
class Agent:
    def __init__(
        ...
        self.optimizer = torch.optim.RMSprop(
            self.network.parameters(), lr=0.0005)  # todo,学习率衰减
        self.scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer=self.optimizer, step_size=1000, gamma=0.9)
    ...
    def transform_state(self, state):
        ...
        def transform_one(state):
            new = np.zeros(shape=(1, 16, matrix_size, matrix_size), dtype=np.float32)
    ...
    def train(self, mode, modelpath=None):
        ...
        while self.eposide < self.max_eposide:
            ...
            while True:
                ...
                (local_steps, self.total_steps) = (
                    local_steps+1, self.total_steps+1)  # 局部计数器和全局计数器加一
            ...
            if game.score if game.score > self.max_score else self.max_score:
                torch.save(self.network.state_dict(),
                           f'./checkpoints/best_{game.maxnum}.pkl')
            ...
            if self.eposide % 100 == 0:
                # print("\nEposide{} finished with score:{} maxnumber:{} steps={}".format(self.eposide,game.score,game.maxnum,local_steps))
                print("max score:{:4d} max number:{:4d}".format(
                    self.max_score, self.max_num))
            writer.add_scalar('max score', self.max_score, self.eposide)
            writer.add_scalar('max number', self.max_num, self.eposide)
            writer.add_scalar('game score', game.score, self.eposide)
            writer.add_scalar('game maxnum', game.maxnum, self.eposide)
            ...
            #更新最优参数
            self.eposide += 1
        torch.save(self.network.state_dict(), './checkpoints/finally.pkl')
